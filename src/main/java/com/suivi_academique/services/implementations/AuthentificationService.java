...
public class AuthentificationService implements AuthentificationInterface {
        ...
        // Logger pour journaliser les opérations du service Authentification
        ...
        public AuthResponse authenticate(PersonnelDTO request){
            // Journalisation du début de l'authentification
            logger.info("Début de l'authentification pour le login : {}", request.getLoginPersonnel());
            ...
            try {
                ...
                logger.info("Authentification réussie pour le personnel : {}", p.getCodePersonnel());
                ...
                logger.error("Erreur lors de l'authentification pour le login {} : {}", request.getLoginPersonnel(), e.getMessage(), e);
                throw e;
            }
        }

        public AuthResponse register(PersonnelDTO personnelDTO){
            // Journalisation du début de l'inscription
            logger.info("Début de l'inscription pour le login : {}", personnelDTO.getLoginPersonnel());

            try {
                if (personnelDTO.getPadPersonnel() == null || personnelDTO.getPadPersonnel().isEmpty()) {
                    // Journalisation de l'erreur de mot de passe vide
                    logger.error("Mot de passe vide pour l'inscription du login : {}", personnelDTO.getLoginPersonnel());
                ...
                if (generatedCode == null) {
                    ...
                    logger.error("Impossible de générer un code pour le rôle : {} pour le login : {}", personnelDTO.getRolePersonnel(), personnelDTO.getLoginPersonnel());
                ...
                try {
                    ...
                    // Journalisation de l'erreur de rôle invalide
                    logger.error("Rôle invalide : {} pour le login : {}", personnelDTO.getRolePersonnel(), personnelDTO.getLoginPersonnel());
                ...
                logger.info("Inscription réussie pour le personnel : {}", savedPersonnel.getCodePersonnel());
                ...
                logger.error("Erreur lors de l'inscription pour le login {} : {}", personnelDTO.getLoginPersonnel(), e.getMessage(), e);
                throw e;
            }
        }
}
